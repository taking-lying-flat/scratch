...
## 1. 先看完整结果：三条多模态序列如何进入同一次前向

假设当前有三条已经完成模板编码和视觉占位展开的样本

| 逻辑序列 | 模态 | 进入语言主干的布局 | 最终长度 | 边界位置 |
| --- | --- | --- | ---: | --- |
| `A` | 图像 + 文本 | `I_A I_A T_A T_A T_A` | 5 | `0 1 2 3 4` |
| `B` | 视频 + 文本 | `V_B T_B T_B` | 3 | `0 1 2` |
| `C` | 纯文本 | `T_C T_C T_C T_C` | 4 | `0 1 2 3` |
...
这里的 `I_A/V_B` 是模板按照视觉网格展开后的语言主干占位，表示视觉特征最终写入的序列位置，而不是视觉编码器内部未经合并的 `patch`

`padding-free` 把有效位置直接拼成一行

```text
input_ids / inputs_embeds:
I_A I_A T_A T_A T_A | V_B T_B T_B | T_C T_C T_C T_C
...
> [!CAUTION]
> `text_position_ids` 重新从 0 开始，不会自行阻止跨样本注意力。它先生成 `cu_seqlens_q/k`，后者才真正把 `Q/K/V` 切成独立注意力问题；多维 `position_ids` 则继续承载每条样本自己的 `mRoPE` 位置语义
...
| `packing_row()` | 拼接字段 | `A|B|C` | 每条样本的 `text_position_ids` 重置 |
| `_data_collator()` | 张量批次 | `input_ids.shape=[1,12]` | `text_position_ids=[0..4|0..2|0..3]` |
