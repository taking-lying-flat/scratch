...
当两个 `pack` 同时进入一个微批，最终边界仍是：

```text
A | C | B | D
```

每个原始样本都是一个独立注意力问题，而不是每个 `pack` 一个注意力问题。

### 13.2 物理 `batch=1` 不等于逻辑 `batch=1`

```text
input_ids.shape = [1, total_tokens]
logical_batch_size = len(cu_seqlens) - 1
```

模型张量只保留一个物理行，逻辑批次由累计边界数量恢复。

### 13.3 `max_seqlen` 不等于 `total_tokens`
...
`total_tokens` 决定连续缓冲区有多少行；`max_seqlen` 描述其中最长的逻辑序列。

### 13.4 `causal` 不负责样本隔离

普通全局 `causal mask` 只禁止读取未来位置。把 `A|B` 当成一条长序列时，`B` 仍然可以读取位于它之前的整个 `A`。必须先由 `cu_seqlens` 切分序列，再在每个切片内部应用 `causal`。

### 13.5 `position_ids` 重置不等于内核已经收到边界

只有当框架确实选择 `varlen` 分支，并将重置位置转换为 `cu_seqlens` 后，样本才在注意力层隔离。调试时不能只打印 `position_ids`，还必须检查最终传给注意力函数的四个字段：
...
cu_seqlens[0] == 0
cu_seqlens[-1] == input_ids.shape[-1]
all(diff(cu_seqlens) > 0)
max(diff(cu_seqlens)) == max_seqlen
labels.shape == input_ids.shape
loss_scale.shape == input_ids.shape  # 存在时
```

### 14.2 进入 `FlashAttention` 前
...
q/k/v are on the same device
cu_seqlens_q/k are contiguous int32 CUDA tensors
```
...
如果边界正确，`B` 对应位置的注意力输出不应随 `A` 内容变化。这个测试比只比较总 `loss` 更直接，因为它专门检测跨样本可见性。

### 14.4 补齐路径与 `padding-free` 路径对照

使用相同模型、相同样本和关闭随机性的配置，分别运行：

```text
normal padded batch + attention_mask
padding-free batch + cu_seqlens
```
...
```text
expanded visual placeholder count
  == visual embeddings count written by masked_scatter

text_position_ids reset count
  == logical multimodal sample count

cu_seqlens[-1]
  == packed LLM-visible sequence length

mRoPE position_ids last dimension
  == packed inputs_embeds sequence dimension
```
